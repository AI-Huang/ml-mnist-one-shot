# MNIST One-Shot LeNet 复现报告

本仓库复现了 MNIST one-shot 设置下的 LeNet 分类实验:每个数字类别仅选取 1 张训练样本,共 10 张原始训练样本。实验重点比较无数据增强的 LeNet baseline 与加入 1024 张几何增强样本后的 LeNet 表现。

原始 README 已移动到 [docs/origin.md](docs/origin.md)。更完整的实验记录见 [report/lenet_reproduction_report.md](report/lenet_reproduction_report.md)。
...
原始 README 中给出了同样 3 个 seed 的 LeNet 结果。本次复现与原始结果的总体趋势一致:无增强 LeNet 约在 0.43-0.45 区间,加入 1024 张几何增强样本后提升到 0.55 左右。

### 5.1 LeNet with Dropout

| seed | 原始 README | 本次复现 | 差值 |
| --- | --- | --- | --- |
...
主要差异来自 `seed=317`:本次复现比原始 README 高 0.0443。其余两个 seed 与原始结果非常接近,差异都小于 0.004。
...
增强组的平均值与原始 README 很接近,本次复现高 0.0061。逐 seed 看,`31` 和 `317` 略高于原始结果,`31731` 略低于原始结果。

### 5.3 数据增强收益对照

| 来源 | 无增强平均 | 增强平均 | 增强收益 |
| --- | --- | --- | --- |
| 原始 README | 0.4294 | 0.5467 | +0.1173 |
| 本次复现 | 0.4460 | 0.5528 | +0.1068 |
| 差值 | +0.0166 | +0.0061 | -0.0105 |

本次复现的增强收益比原始 README 低 0.0105,原因不是增强组明显退化,而是无增强 baseline 在本次复现中更高。总体结论不变:几何数据增强仍然带来约 10-12 个百分点的稳定提升。
...
1. **无增强 LeNet 仍能达到约 44.6% 平均准确率**,略高于原始 README 中的 42.94%。说明在 10-shot 条件下,LeNet + Dropout 有一定泛化能力,但仍明显受限于样本数量。
...
本项目的复现工作参考了 borgwang 的开源实现 [toys/ml-mnist-one-shot](https://github.com/borgwang/toys/tree/master/ml-mnist-one-shot)。原项目提供了 MNIST one-shot 实验设定、基线结果与数据增强思路,为本仓库的代码整理、实验复现和结果对照提供了重要参考。

## 8. References

- borgwang, [toys/ml-mnist-one-shot](https://github.com/borgwang/toys/tree/master/ml-mnist-one-shot).
- Guibo Wang, [MNIST One-Shot 学习实验记录](https://zhuanlan.zhihu.com/p/94368616), 知乎专栏.
